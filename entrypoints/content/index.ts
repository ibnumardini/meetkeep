...
export default defineContentScript({
  ...
  main() {
    ...
    let elapsedAtLeave: number | null = null;
    ...
    function formatElapsedWords(ms: number, lang: 'id' | 'en') {
      const totalSeconds = Math.floor(ms / 1000);
    ...
    function detectLang(): 'id' | 'en' | null {
    ...
    function isLeaveTitle(text: string) {
    ...
    function buildTimerElement() {
      ...
      label.addEventListener('mouseenter', () => {
        ...
        label.classList.add('meetkeep-label--hover');
      });
      label.addEventListener('mouseleave', () => {
        label.textContent = customLabel;
        label.classList.remove('meetkeep-label--hover');
      });
    ...
    function injectTimer(target: HTMLElement) {
